...
int main(int argc, char *argv[])
{
    ...
    while ( (c = getopt(argc, argv, "b:c:dhi:l:p:r:s:S:")) != -1) {
        switch (c) {
            case 'b':
    ...
    for (int i = 0; i < vlen; ++i) {
        ...
        iov[i]->iov_base = malloc(arg_to_server.bufsize);
        memset(iov[i]->iov_base, 0, arg_to_server.bufsize);
        iov[i]->iov_len = arg_to_server.bufsize;
        msgvec[i].msg_hdr.msg_iov = iov[i];
        msgvec[i].msg_hdr.msg_iovlen = 1;
        msgvec[i].msg_len = 0;
    }
    // end of data to use recvmmsg()
    ...
    prev = start;
    unsigned long recvmmsg_count = 1;
    for ( ; ; ) {
        //if (has_alarm) {
        if (recvmmsg_count % 1000 == 0) {
            has_alarm = 0;
        ...
        n = recvmmsg(sockfd, msgvec, vlen, 0, NULL);
        if (n < 0) {
        ...
        for (int i = 0; i < n; ++i) {
            interval_read_bytes += msgvec[i].msg_len;
            total_read_bytes    += msgvec[i].msg_len;
            seq_num = get_seq_num(msgvec[i].msg_hdr.msg_iov->iov_base, msgvec[i].msg_len);
